...
fn get_mut_gram_by_u8<'a, T>(grambuf: &mut [u8]) -> &'a mut T {
    // 将字节切片转换为对 T 类型的引用
    unsafe {
        &mut *(grambuf[0..].as_mut_ptr() as *mut T)
    }
...
impl MessageDataGram {
    ...
    pub fn is_message_from_bytes(datas: &[u8]) -> bool {
        if datas.len() >= Self::get_size() { Self::is_message(datas) } else { false }
    }
    ...
    pub fn create_gram_buf<'a>(datasize: usize) -> Vec<u8> {
        ...
        let mut vec_u8: Vec<u8> =
            vec![0x00; datasize + size_of_align_data_gram::<MessageDataGram>()];
        vec_u8
    }
    pub fn create_message_data_gram_by_mut_vec8<'a>(
        byte_array: &'a mut Vec<u8>
    ) -> &'a mut MessageDataGram {
        let grambuf: &mut [u8] = byte_array.as_mut_slice();
        MessageDataGram::create_message_data_gram_by_mut_u8(grambuf)
    }
    pub fn create_message_data_gram_by_mut_u8<'a>(
        grambuf: &'a mut [u8]
    ) -> &'a mut MessageDataGram {
        #[allow(unused_mut)]
        let mut data_gram_ref: &mut MessageDataGram =
            get_mut_gram_by_u8::<MessageDataGram>(grambuf); //unsafe {&mut *(grambuf[0..].as_mut_ptr() as *mut DataGram)};
        // 设置结构体下,缓冲区的大小
        data_gram_ref.set_bitcomm(BitcommFlag::BITCOMM_MESSAGE);
        data_gram_ref.set_version(BitcommVersion::BITCOMM_VERSION_0_1_0_1);
        data_gram_ref.datasize = (grambuf.len() -
            size_of_align_data_gram::<MessageDataGram>()) as u32;
        data_gram_ref
    }
    ...
    pub fn create_message_data_gram_from_mdg_u8<'a>(
        grambuf: &'a mut [u8],
        value: &'a MessageDataGram
    ) -> &'a mut MessageDataGram {
        #[allow(unused_mut)]
...
impl CommandDataGram {
    ...
    pub fn is_bitcomm_flag(datas: &[u8]) -> bool {
        if datas.len() == std::mem::size_of::<BitcommFlag>() {
            let bitcomm_flag = get_gram_by_u8::<u32>(datas);
    ...
    pub fn is_command_from_bytes(datas: &[u8]) -> bool {
        if datas.len() == Self::get_size() { Self::is_command(datas) } else { false }
    }
    ...
    pub fn create_command_data_gram_by_mut_vec8<'a>(
        byte_array: &'a mut Vec<u8>
    ) -> &'a mut CommandDataGram {
        let grambuf: &mut [u8] = byte_array.as_mut_slice();
        Self::create_command_data_gram_by_mut_u8(grambuf)
    }
    pub fn create_command_data_gram_by_mut_u8<'a>(
        grambuf: &'a mut [u8]
    ) -> &'a mut CommandDataGram {
        #[allow(unused_mut)]
        let mut data_gram_ref: &mut CommandDataGram =
            get_mut_gram_by_u8::<CommandDataGram>(grambuf); //unsafe {&mut *(grambuf[0..].as_mut_ptr() as *mut DataGram)};
        // 设置结构体标志
        data_gram_ref.datasize = (grambuf.len() -
            size_of_align_data_gram::<CommandDataGram>()) as u32;
    ...
    pub fn create_gram_buf<'a>(datasize: usize) -> Vec<u8> {
        ...
        #[allow(unused_mut)]
        let mut vec_u8: Vec<u8> =
            vec![0x00; datasize + size_of_align_data_gram::<CommandDataGram>()];
        vec_u8
    }
    pub fn create_command_gram_from_gram<'a>(
        buf: &'a mut [u8],
        value: &'a CommandDataGram
    ) -> &'a mut CommandDataGram {
...
pub enum InnerDataGram {
    ...
    }, //rescmdbuff:RefCell<BytesMut>,rescmdgram:RefCell<CommandDataGram>},
    Message {
        reqmsgbuff: Arc<Bytes>,
        reqmsggram: Arc<MessageDataGram>,
    }, //,resmsgbuff:RefCell<BytesMut>,resmsggram:RefCell<MessageDataGram>},
